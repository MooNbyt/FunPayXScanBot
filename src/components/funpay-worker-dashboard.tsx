...
export default function FunPayWorkerDashboard() {
  ...
  const [scraperBanPauseMinutes, setScraperBanPauseMinutes] = useState(30);
  const [scraperBatchSize, setScraperBatchSize] = useState(50);
  const [scraperWriteBatchSize, setScraperWriteBatchSize] = useState(50);
  const [projectLogsTtl, setProjectLogsTtl] = useState(1);
  ...
  const handleSaveConfig = useCallback(async (newConfig: any, requireUnlock: boolean = false) => {
    if (requireUnlock && !isSettingsUnlocked) {
        toast({
            variant: "destructive",
            title: "Доступ запрещен",
            description: "Разблокируйте настройки для сохранения системных параметров.",
        });
        return;
    }
    try {
  ...
  }, [toast, isSettingsUnlocked]);

  const handleConfigChange = (updater: Function, value: any, configKey: string, requireUnlock: boolean = false) => {
      updater(value);
      ...
      debounceTimeoutRef.current = setTimeout(() => {
          ...
          if (configKey === 'SCRAPER_PAUSE_DURATION_MS') {
              valueToSave = value * 60 * 60 * 1000;
          }
          handleSaveConfig({ [configKey]: valueToSave }, requireUnlock);
      }, 1000);
  };
  ...
  const fetchConfig = useCallback(async () => {
      try {
        ...
        if (response.ok) {
          ...
          setScraperBanPauseMinutes(data.SCRAPER_BAN_PAUSE_MINUTES || 30);
          setScraperBatchSize(data.SCRAPER_BATCH_SIZE || 50);
          setScraperWriteBatchSize(data.SCRAPER_WRITE_BATCH_SIZE || 50);
          setProjectLogsTtl(data.PROJECT_LOGS_TTL_HOURS || 1);
  ...
  }, [activeTab, checkScrapingStatus, fetchTelegramLogs, fetchProducts, fetchProjectLogs, isSettingsUnlocked, fetchDashboardData]);
  ...
  const handleConfirmImport = async () => {
    ...
    const handleSetWebhook = async () => {
      ...
      const configToSave = { 
        ...
        TELEGRAM_WELCOME_MESSAGE: telegramWelcome,
        TELEGRAM_WELCOME_IMAGE_URL: telegramWelcomeImageUrl,
      };

      if (debounceTimeoutRef.current) {
          clearTimeout(debounceTimeoutRef.current);
      }
       await handleSaveConfig(configToSave, false);
  ...
  const handleInsertVariable = (variable: string) => {
    ...
    if (textarea) {
      ...
      handleConfigChange(setTelegramWelcome, newText, 'TELEGRAM_WELCOME_MESSAGE', false);
  ...
  const handleUnlockSettingsSubmit = async (e: React.FormEvent) => {
    ...
    const handleAddCategory = async () => {
      ...
      if(existingCategories.includes(newCategory)) {
        toast({ variant: 'destructive', title: 'Ошибка', description: 'Категория с таким названием уже существует.' });
        return;
      }
      setCurrentCategoryView(newCategory);
      setNewCategory("");
      setIsAddingCategory(false);
      toast({ title: 'Категория создана', description: `Теперь вы можете добавлять товары в категорию "${newCategory}".` });
  };
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
                <Dialog open={isWorkerManagerOpen} onOpenChange={setIsWorkerManagerOpen}>
                  <DialogTrigger asChild disabled={!isSettingsUnlocked}>
                      <Card className={`bg-secondary h-full ${isSettingsUnlocked ? 'cursor-pointer hover:bg-accent transition-colors' : 'cursor-not-allowed'}`}>
                        <CardHeader className="pb-2 flex flex-row items-center justify-between">
                ...
                <Card className="bg-secondary h-full">
                 ...
                 <Card className="bg-secondary h-full">
                    <CardHeader className="pb-2 flex flex-row items-center justify-between">
                    <CardDescription>Ошибки 404</CardDescription>
                             ...
                             {!currentCategoryView && (
                                <Dialog open={isAddingCategory} onOpenChange={setIsAddingCategory}>
                                    ...
                                    <DialogContent>
                                        ...
                                        <DialogFooter>
                                            <Button variant="secondary" onClick={() => setIsAddingCategory(false)}>Отмена</Button>
                    ...
                    <CardContent>
                       <ScrollArea className="h-[500px]">
                           {currentCategoryView === null ? (
                                // Root view: Categories and products without category
                                <div>
                                    {categories.length > 0 && (
                                      <div className="mb-6">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            {categories.map(cat => (
                                                <Button variant="outline" key={cat} onClick={() => setCurrentCategoryView(cat)} className="h-20 flex-col gap-2">
                                ...
                                // Category view: products in current category
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead>Название</TableHead>
                 ...
                 <Card className="bg-secondary">
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input id="telegram-token" placeholder="123456:ABC-DEF1234..." value={telegramToken} onChange={(e) => handleConfigChange(setTelegramToken, e.target.value, 'TELEGRAM_TOKEN')} className="bg-card"/>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                              <Label htmlFor="telegram-provider-token">Токен провайдера платежей</Label>
                              <Input id="telegram-provider-token" placeholder="Live или Test токен от @BotFather" value={telegramProviderToken} onChange={(e) => handleConfigChange(setTelegramProviderToken, e.target.value, 'TELEGRAM_PROVIDER_TOKEN')} className="bg-card"/>
                           ...
                           <div className="space-y-2">
                                ...
                                <Select value={telegramPaymentCurrency} onValueChange={(value) => handleConfigChange(setTelegramPaymentCurrency, value, 'TELEGRAM_PAYMENT_CURRENCY')} disabled={!telegramProviderToken}>
                         ...
                         <div className="space-y-2">
                          ...
                          <Input id="telegram-shop-button-name" placeholder="Магазин" value={telegramShopButtonName} onChange={(e) => handleConfigChange(setTelegramShopButtonName, e.target.value, 'TELEGRAM_SHOP_BUTTON_NAME')} className="bg-card"/>
                        </div>
                        ...
                        <div className="p-4 bg-card rounded-lg border space-y-4">
                            <div className="flex flex-row items-center justify-between">
                              ...
                              <Switch
                                ...
                                onCheckedChange={(checked) => handleConfigChange(setTelegramPaymentEnabled, checked, 'TELEGRAM_PAYMENT_ENABLED')}
                              />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                               <div className="space-y-2">
                                 <Label htmlFor="telegram-search-cost">Стоимость в Telegram Stars</Label>
                                 <Input id="telegram-search-cost" type="number" min="1" placeholder="1" value={telegramSearchCost} onChange={(e) => handleConfigChange(setTelegramSearchCost, e.target.value, 'TELEGRAM_SEARCH_COST_STARS')} className="bg-background" disabled={!telegramPaymentEnabled}/>
                               </div>
                                <div className="space-y-2">
                                 <Label htmlFor="telegram-search-cost-real">Стоимость в {telegramPaymentCurrency}</Label>
                                 <Input id="telegram-search-cost-real" type="number" min="1" placeholder="10" value={telegramSearchCostReal} onChange={(e) => handleConfigChange(setTelegramSearchCostReal, e.target.value, 'TELEGRAM_SEARCH_COST_REAL')} className="bg-background" disabled={!telegramPaymentEnabled || !telegramProviderToken}/>
                               </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-card rounded-lg border space-y-4">
                            <div className="flex flex-row items-center justify-between">
                              ...
                              <Switch
                                ...
                                onCheckedChange={(checked) => handleConfigChange(setTelegramConnectionPaymentEnabled, checked, 'TELEGRAM_CONNECTION_PAYMENT_ENABLED')}
                              />
                            </div>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                               <div className="space-y-2">
                                 <Label htmlFor="telegram-connection-cost">Стоимость в Telegram Stars</Label>
                                 <Input id="telegram-connection-cost" type="number" min="1" placeholder="5" value={telegramConnectionCost} onChange={(e) => handleConfigChange(setTelegramConnectionCost, e.target.value, 'TELEGRAM_CONNECTION_COST_STARS')} className="bg-background" disabled={!telegramConnectionPaymentEnabled}/>
                               </div>
                               <div className="space-y-2">
                                 <Label htmlFor="telegram-connection-cost-real">Стоимость в {telegramPaymentCurrency}</Label>
                                 <Input id="telegram-connection-cost-real" type="number" min="1" placeholder="50" value={telegramConnectionCostReal} onChange={(e) => handleConfigChange(setTelegramConnectionCostReal, e.target.value, 'TELEGRAM_CONNECTION_COST_REAL')} className="bg-background" disabled={!telegramConnectionPaymentEnabled || !telegramProviderToken}/>
                               </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                           ...
                           <Input id="telegram-welcome-image" placeholder="https://example.com/image.png" value={telegramWelcomeImageUrl} onChange={(e) => handleConfigChange(setTelegramWelcomeImageUrl, e.target.value, 'TELEGRAM_WELCOME_IMAGE_URL')} className="bg-card"/>
                         </div>
                         <div className="space-y-2">
                            <Label htmlFor="telegram-welcome">Приветственное сообщение</Label>
                            <Textarea ref={welcomeMessageRef} id="telegram-welcome" placeholder="Введите приветствие..." value={telegramWelcome} onChange={(e) => handleConfigChange(setTelegramWelcome, e.target.value, 'TELEGRAM_WELCOME_MESSAGE')} className="bg-card h-24"/>
                             <div className="flex gap-2 flex-wrap">
                                <Button variant="outline" size="sm" onClick={() => handleInsertVariable('{user_count}')}>&#123;user_count&#125;</Button>
                            </div>
            ...
            <TabsContent value="telegram-logs">
                <Card className="bg-secondary">
                    <CardHeader>
                        ...
                        <CardDescription>Здесь отображаются последние входящие запросы от Telegram.</CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <TabsContent value="settings">
                  <Card className="bg-secondary">
                    ...
                    <CardContent className="space-y-6">
                      ...
                      <div>
                        <h3 className="text-lg font-medium text-foreground mb-4">Настройки Скрейпера</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div className="space-y-2">
                                <Label htmlFor="scraper-batch-size">Размер пачки для парсинга</Label>
                                <Input id="scraper-batch-size" type="number" placeholder="50" value={scraperBatchSize} onChange={(e) => handleConfigChange(setScraperBatchSize, e.target.value, 'SCRAPER_BATCH_SIZE', true)} className="bg-card"/>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="scraper-write-batch-size" type="number" placeholder="50" value={scraperWriteBatchSize} onChange={(e) => handleConfigChange(setScraperWriteBatchSize, e.target.value, 'SCRAPER_WRITE_BATCH_SIZE', true)} className="bg-card"/>
                             ...
                             <div className="space-y-2">
                                ...
                                <Input id="recent-profiles-limit" type="number" placeholder="100" value={scraperRecentProfilesLimit} onChange={(e) => handleConfigChange(setScraperRecentProfilesLimit, e.target.value, 'SCRAPER_RECENT_PROFILES_LIMIT', true)} className="bg-card"/>
                                <p className="text-xs text-muted-foreground">Макс. кол-во профилей в логе на главной.</p>
                            </div>
                             <div className="space-y-2">
                                <Label htmlFor="ban-pause-duration">Пауза при ошибке/бане (минуты)</Label>
                                <Input id="ban-pause-duration" type="number" placeholder="30" value={scraperBanPauseMinutes} onChange={(e) => handleConfigChange(setScraperBanPauseMinutes, e.target.value, 'SCRAPER_BAN_PAUSE_MINUTES', true)} className="bg-card"/>
                                <p className="text-xs text-muted-foreground">Пауза при временных ошибках (не 404).</p>
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="consecutive-error-limit">Лимит ошибок 404</Label>
                                <Input id="consecutive-error-limit" type="number" placeholder="100" value={scraperConsecutiveErrorLimit} onChange={(e) => handleConfigChange(setScraperConsecutiveErrorLimit, e.target.value, 'SCRAPER_CONSECUTIVE_ERROR_LIMIT', true)} className="bg-card"/>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="pause-duration" type="number" placeholder="6" value={scraperPauseDuration} onChange={(e) => handleConfigChange(setScraperPauseDuration, e.target.value, 'SCRAPER_PAUSE_DURATION_MS', true)} className="bg-card"/>
                                <p className="text-xs text-muted-foreground">На сколько часов остановиться после лимита 404.</p>
                            </div>
                             <div className="space-y-2">
                                <Label htmlFor="project-logs-ttl">Время жизни обычных логов (часы)</Label>
                                <Input id="project-logs-ttl" type="number" placeholder="1" value={projectLogsTtl} onChange={(e) => handleConfigChange(setProjectLogsTtl, e.target.value, 'PROJECT_LOGS_TTL_HOURS', true)} className="bg-card"/>
                                <p className="text-xs text-muted-foreground">Через сколько часов удалять обычные логи проекта.</p>
                            </div>
                        </div>
                      </div>
                      <Separator />
