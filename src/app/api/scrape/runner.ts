...
export async function runScraperProcess(workerId: string) {
    if (!workerId) {
        await log(`[Scraper Runner] CRITICAL: Scraper started without a workerId. Exiting.`);
        return;
    }
    ...
    if (!REDIS_URI || !MONGODB_URI) {
        ...
        // Clean up status key if DB is not configured
    ...
    try {
        ...
        // The manager now sets the status key before calling this function.
